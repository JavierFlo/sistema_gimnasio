    ...
    <SCRIPT LANGUAGE="JavaScript">
		function checkIt(evt) {
		    ...
		    if (charCode > 31 && (charCode < 48 || charCode > 57)) {
		        status = "Solo se aceptan numeros ."
		        return false
		    }
		    status = ""
		    return true
					...
					<?php
						...
						if (mysqli_affected_rows($con) != 0) {
						    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
						        ...
						        echo "<td><form action='bal_pay.php' method='post'><input type='hidden' name='name' value='" . $row['invoice'] . "'/><input type='submit' value='Balance de pago ' class='btn btn-info'/></form></td></tr>";
						        $msgid  = 0;
