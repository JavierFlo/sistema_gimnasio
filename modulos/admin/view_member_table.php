				...
				<?php
					...
					if (mysqli_affected_rows($con) != 0) {
					    while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
					        ...
					        echo "<td><form action='del_member_table.php' method='post' onSubmit='return ConfirmDelete();'><input type='hidden' name='name' value='" . $mem_id . "'/><input type='submit' value='Eliminar horario' class='btn btn-danger'/></form><form action='print_member_table.php' method='post'><input type='hidden' name='name' value='" . $mem_id . "'/><input type='hidden' name='full_name' value='" . $row['name'] . "'/><input type='hidden' name='details' value='" . $row['details'] . "'/><input type='hidden' name='date' value='" . $row['date'] . "'/><input type='submit' value='Imprimir horario' class='btn btn-info'/></form></td></tr>";
					        $msgid = 0;
					    }
					}
